...
for (lakes in unique(df$LakeName)){
  ...
  for (dates in unique(input$Date)){
    ...
    if (length(input_date$Depth_m) <= 1){
      next
    }
    
    res = schmidt.stability_idso(wtr = input_date$Temperature_degCelsius, depths = input_date$Depth_m, bthA = hypsography$Area.at.z, bthD = hypsography$depths)
    
    bf = center.buoyancy(wtr =  input_date$Temperature_degCelsius, depths = input_date$Depth_m)
    
    results = rbind(results, data.frame('lake' = lakes,
                                        'Datetime' =  dates,
                                        'St' = res$St,
                                        'mean_rho' = res$rho,
                                        'Zg' = res$z_g,
                                        ...
                                        'lake_area' =  mean(input$SurfaceArea_km2 * 1e6),
                                        'surface_temp' = input_date$Temperature_degCelsius[which(input_date$Depth_m == min(input_date$Depth_m))],
                                        'bottom_temp' = input_date$Temperature_degCelsius[which(input_date$Depth_m == max(input_date$Depth_m))],
                                        'therm_dep' = bf))
...
write.csv(file = "../data_processed/pilla.csv", x = results, quote = F, row.names = F)
